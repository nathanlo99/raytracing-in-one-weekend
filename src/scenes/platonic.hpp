...
inline auto platonic_scene() {
  ...
  world.add(
      std::make_shared<sphere>(point3(0, -1001, 0), 1000, ground_material));
  ...
  world.add(std::make_shared<sphere>(point3(-2.5, 0, 0), 1.0, egg_material));

  const auto mirror_material =
      material_manager::create<metal>(colour(0.7, 0.6, 0.5), 0.1);
  world.add(std::make_shared<sphere>(point3(2.5, 0, 0), 1.0, mirror_material));
